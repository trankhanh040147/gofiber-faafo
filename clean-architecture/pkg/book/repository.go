...
func (r *repository) CreateBook(book *entities.Book) (*entities.Book, error) {
	...
	if mongo.IsDuplicateKeyError(err) {
		return nil, errors.New("Error duplicated key")
	} else {
		return nil, err
	}
...
func (r *repository) CreateBooks(books []*entities.Book) ([]*entities.Book, error) {
	var data []interface{}
	for _, book := range books {
		data = append(data, book)
	}

	results, err := r.Collection.InsertMany(context.Background(), data)
