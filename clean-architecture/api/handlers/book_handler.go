...
import (
	...
	"gofiber-faafo/api/presenter"
	entities "gofiber-faafo/entities/book"
...
func AddBook(service book.Service) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		_, err = service.InsertBook(&requestBody)
		//fmt.Println(result)
		if err != nil {
			c.Status(http.StatusInternalServerError)
			return c.JSON(presenter.BookErrorResponse(err))
		}
		//return c.JSON(presenter.BookSuccessResponse(nil))
		return c.JSON(true)
	}
}

// AddBooks is handler/controller which creates Books in the BookShop
func AddBooks(service book.Service) fiber.Handler {
	return func(c *fiber.Ctx) error {
		var requestBody entities.Book
		err := c.BodyParser(&requestBody)
		if err != nil {
		...
		bookNew := entities.Book{
			Title:  "Book New",
			Author: "N/A",
		}
		books = append(books, &requestBody, &bookNew)

		result, err := service.InsertBooks(books)
		if err != nil {
			c.Status(http.StatusInternalServerError)
			return c.JSON(presenter.BookErrorResponse(err))
		}
		return c.JSON(presenter.BookSuccessResponse(result[0]))
	}
}
